...
updn <- updn %>%
  ...
  rename_all(funs(gsub('[[:punct:] ]+','_',colnames(updn)))) %>%
...
bb <- ggplot(exp1, aes(x = trt, fill  = bee_wind_orientation)) + 
  geom_bar(position = "identity", alpha = 0.5) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) + 
  geom_hline(aes(yintercept =  0.5)) + 
  scale_fill_viridis_d() 
...
m1 <- glm(fly_upwind ~ (side  +  flight_direction + day)^2, data = exp1, family = binomial("logit"))

summary(m1)

alias((lm(as.numeric(fly_upwind) ~ (side  +  flight_direction + day)^2, data = exp1)), partial = TRUE)
car::vif((lm(as.numeric(fly_upwind) ~ (side  +  flight_direction + day)^2, data = exp1)))
...
preddf <- expand.grid(side = unique(exp1$side), flight_direction = unique(exp1$flight_direction), 
                      day = unique(exp1$day), bee_wind_orientation = unique(exp1$bee_wind_orientation))
...
m1.1 <- glm(Freq ~ (bee_wind_orientation + flight_direction + side)^2 + offset(log(day_side_total)), data = posdf2, family = poisson("log"))
summary(m1.1)
...
finMod = m1.5
summary(finMod) # weird that bees are more likely to fly towards feeder
...
pds <- predict(m1.5,newdata = preddf2, type = "link", se.fit = TRUE)
...
e1Plot <- ggplot(pp1, aes(x = flight_direction, y = proportion, color = dataType))+ 
  facet_grid(.~bee_wind_orientation ) + 
...
ggplot(exp3, aes(x = interaction(side, fast_slow, flight_direction), fill = side)) + 
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(exp3, aes(x = interaction(flight_direction), fill = side)) + 
  geom_bar() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
