...
# here -- confused
cc <- ggplot(exp1, aes(x = flight.direction)) + 
  geom_bar(aes( fill = bee.wind.orientation), stat="count") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5)) + 
  geom_hline(aes(yintercept =  0.5), linetype = 2) + 
  scale_fill_viridis(discrete = TRUE) + 
  facet_grid(side ~ day , labeller = labeller(.rows = label_both, .cols = label_both))
cc
